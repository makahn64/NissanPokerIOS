...
namespace {
    ...
    ADQCARApplicationSession* instance = nil;
    ...
    NSString * ADQCAR_APPLICATION_ERROR_DOMAIN = @"appd_vuforia_app";
}

@interface ADQCARApplicationSession ()
...
@implementation ADQCARApplicationSession
@synthesize viewport;

- (id)initWithDelegate:(id<ADQCARApplicationControl>) delegate
{
    self = [super init];
    if (self) {
...
- (NSError *) NSErrorWithCode:(int) code {
    return [NSError errorWithDomain:ADQCAR_APPLICATION_ERROR_DOMAIN code:code userInfo:nil];
}

- (BOOL) NSErrorWithCode:(int) code error:(NSError **) error{
    if (error != NULL) {
...
- (void) errorMessage:(NSString *) message {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:ADQCAR_APPLICATION_ERROR_DOMAIN
                                                    message:message
                                                   delegate:nil
